---
name: manciple-task-planner
description: Plan and create Manciple task specs from a product brief, feature description, GitHub issue, or engineering goal. Use when the user asks Claude Code to plan, decompose, break down, or create tasks for a feature or goal — especially before any implementation starts.
---

# Manciple Task Planner

Receive a goal or feature description and produce a set of valid, fully-specified Manciple task specs in `.manciple/tasks/active/`. Do not implement the work. Your output is the plan.
...
- Which directories must not be touched (admin interfaces, unrelated services, generated files, build outputs)
 - Which domain label fits — check `.manciple/specs/domains/` if it exists; otherwise infer from directory structure (`auth`, `core`, `api`, `ui`, `infra`, etc.)
...
When MCP tools are available, call `manciple_create` for each task. The tool generates the `id` from the title via slugify and validates the spec before writing it.
...
If `manciple_create` returns an error because the id already exists, report the collision and ask whether to skip or choose a different title.

If MCP tools are unavailable, fall back to writing YAML files directly to `.manciple/tasks/active/<task-id>.yaml` using the same fields. Do not use `manciple new` — it stubs required fields.
...
After writing all specs, run `manciple_validate` (or `manciple validate` via CLI). Fix every reported error before finishing. Do not stop at warnings — resolve them or document why they are acceptable.
