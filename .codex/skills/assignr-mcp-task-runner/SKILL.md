...
description: Complete implementation, docs, test, refactor, review, or hardening work from Assignr task specs using the Assignr MCP server. Use when the user asks Codex to run, complete, pick up, execute, or work through an Assignr task in this repo, especially when MCP tools named assignr_list, assignr_get_task, assignr_compile, assignr_set_status, assignr_run_log, assignr_validate, or assignr_get_compiled_prompt are available.
---
