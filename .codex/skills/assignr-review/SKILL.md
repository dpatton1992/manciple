---
name: assignr-review
description: Review completed Assignr task work by loading the task packet, checking review readiness with assignr_verify --profile review, evaluating acceptance criteria against run log and git diff evidence, and recording a deterministic verdict (approve/request-changes/block). Use when the user asks Codex to review, approve, or give feedback on a completed Assignr task.
---
